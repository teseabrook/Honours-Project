...
HiltWrapMesh::HiltWrapMesh(ID3D11Device* device, ID3D11DeviceContext* deviceContext, ParameterSet* a_set)
{
	set = a_set;
	initBuffers(device);
}
...
void HiltWrapMesh::createCirclePoints(XMFLOAT3 centre, float radius)
{
...
void HiltWrapMesh::generateSpiralPoints(XMFLOAT3 centre, float radius)
{
	int vCounterStart = vCounter;
	...
	for (int i = 0; i < loops; i++)
	{
		...
		float newX = (radius / DEBUG_SCALE_FACTOR) * sin(theta) + centre.x;
		float newY = (radius / DEBUG_SCALE_FACTOR) * cos(theta) + centre.y;
		vertices[vCounter].position = XMFLOAT3(newX, newY, centre.z + (i / 25.0f));
		vertices[vCounter].normal = XMFLOAT3(0.0f, -1.0f, 0.0f);
		vertices[vCounter].texture = XMFLOAT2(0.0f, 0.0f);
